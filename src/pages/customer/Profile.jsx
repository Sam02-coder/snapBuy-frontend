...
export default function Profile() {
  ...
  useEffect(() => {
    customerService.getProfile().then((res) => setProfile(res.data.data)).finally(() => setLoading(false))
  }, [])
...
function ProfileForm({ profile, onUpdated }) {
  ...
  } = useForm({ defaultValues: { fullName: profile?.fullName, phone: profile?.phone } })

  async function onSubmit(values) {
    try {
      const { data } = await customerService.updateProfile(values)
      onUpdated(data.data)
  ...
  return (
    <Card className="p-6">
      <h2 className="font-display text-lg text-navy mb-4">Profile details</h2>
